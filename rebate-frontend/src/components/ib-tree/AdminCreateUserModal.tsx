...
export function AdminCreateUserModal({ onClose }: { onClose: () => void }) {
  ...
  const handleParentChange = async (id: string, email?: string) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        const pTypes = (res.data.accountTypes && res.data.accountTypes.length > 0)
          ? res.data.accountTypes
          : [res.data.accountType || 'STD'];
        ...
        // Auto-inherit accountTypes from parent
        setSelectedAccountTypes(pTypes);
        setAccountType(pTypes[0] || 'STD');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'mib') {
        result = await ibApi.createMib({
          email,
          name,
          password,
          accountType: selectedAccountTypes[0] || 'STD',
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === 'sub-ib' && parentPreview && (
            <div className="rounded-lg bg-blue-50 p-3 text-sm">
              ...
              <p className="text-blue-700">Loại link sở hữu: {parentPreview.accountTypes.join(', ')}</p>
            </div>
          )}

          {/* Account Types selection checkboxes */}
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">
              Cấp các loại tài khoản link
            </label>
            <p className="mb-2 text-xs text-gray-500">
              {mode === 'mib'
                ? 'Chọn các loại link cấp cho MIB mới từ danh sách hệ thống'
                : 'Chọn loại link cấp cho Sub-IB (trong phạm vi Node cha đang sở hữu)'}
            </p>
            <div className="grid grid-cols-2 gap-2 rounded-xl border border-gray-200 p-3 bg-gray-50/50">
              {(mode === 'mib' ? availableMibTypes : (parentPreview?.accountTypes || ['STD'])).map((type) => {
